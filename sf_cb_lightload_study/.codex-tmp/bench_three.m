...
for q = [0.5, 0.1]
    t0 = tic;
    r = simulate_sfcb_lightload_variant('sb_cb', trace, scenario, cfg, M, q, 1001);
    t1 = toc(t0);
    fprintf('BENCH sb_cb M=6 lambda=10 q=%g: %.3f s (attempts=%d)\n', q, t1, r.diagnostics.rts_attempts);
end
for q = [1, 0.5]
    t0 = tic;
    r = simulate_sfcb_lightload_variant('sf_cb', trace, scenario, cfg, M, q, 1001);
    t1 = toc(t0);
    fprintf('BENCH sf_cb M=6 lambda=10 q=%g: %.3f s (attempts=%d)\n', q, t1, r.diagnostics.rts_attempts);
end
end
