...
% packet conservation / delay identity and a physical-layer smoke test.
    ...
    results = check(results, all(abs(comp11s - [364.2; 868.2]) < tol), ...
    ...
    % ---- T12: exponential retry delay distribution ----
    % With a single packet at t=0 and q=0.5, lambda = -log(1-q)/9 =
    % log(2)/9 per us, so the mean delay is 9/log(2) ~= 12.98 us and
    % P(delay < 9 us) = 1 - exp(-lambda*9) = q = 0.5.
    ...
    results = check(results, all(abs(w12 - p12) < tol), ...
        'exponential delay accounting matches observed wait');
    results = check(results, abs(mean(w12) - 9/log(2)) < 2.5, ...
        'exponential delay mean ~= 9/log(2) us at q=0.5');
    results = check(results, abs(mean(w12 < 9) - 0.5) < 0.1, ...
        'P(delay < 9 us) ~= q = 0.5');
