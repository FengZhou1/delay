...
% mode = 'unslotted' -> non-slotted p-persistent ALOHA: a fresh HOL packet
%   draws Bernoulli(q) immediately and transmissions use real durations
%   (RTS=14.5 us, SIFS=16 us, CTS sweep=117.6 us, DATA=M*164.1 us) without
%   any 9 us boundary alignment.  No carrier sensing.
...
% retry after the SIFS+CTS-sweep timeout.
    ...
    rts_us = timing.RTS_US;
    sifs_us = timing.SIFS_US;
    difs_us = timing.DIFS_US;
    cts_us = timing.CTS_US;
    cts_sweep_us = timing.CTS_SWEEP_US;
    conn_slot_us = timing.CONN_SLOT_US;
    cts_timeout_us = timing.CTS_TIMEOUT_US;
    difs_ticks = timing.DIFS_TICKS;
    ...
    scheduled_success = false(n_nodes,1);
    ...
    function enter_hol(u, t_hol)
        if nav_until(u) > t_hol
            node_state(u) = ST_NAV;
            scheduled_success(u) = false;
            sense_count(u) = 0;
            sense_start(u) = nav_until(u);
            next_tick(u) = idle_resume_tick(nav_until(u));
        ...
        elseif strcmp(mode,'sb_cb')
            ...
            % alignment); the DIFS completion instant is derived from
            % sense_start in process_tick, so the Bernoulli draw happens at
            % exactly sense_start + DIFS (34 us).
            ...
            % A fresh HOL packet decides immediately at the arrival instant:
            % Bernoulli(q) right now, without waiting for the next 9 us tick.
    ...
    function flag = process_tick(u, t_now)
        ...
        if node_state(u) == ST_SENSE
            ...
            if t_now >= sense_start(u) + difs_us
                % Continuous idle since sense_start: DIFS (34 us) is
                % complete.  Fall through to the ST_READY processing below,
                % which performs the Bernoulli(q) draw immediately at this
                % continuous instant.
                node_state(u) = ST_READY;
                scheduled_success(u) = false;
                sense_count(u) = 0;
        ...
        if strcmp(mode,'sb_cb')
            ...
            if busy
                ...
                sense_count(u) = 0;
                sense_start(u) = busy_end;
                next_tick(u) = idle_resume_tick(busy_end);
                if next_tick(u) <= t_now
                    next_tick(u) = t_now + slot_us;
                end
                return;
        ...
        if scheduled_success(u)
            scheduled_success(u) = false;
            flag = true;
            return;
        end
        k = geometric_q(q, stream);
        if ~is_saturation
            pid = head_packet_id(u);
            probability_wait_us(pid) = ...
                probability_wait_us(pid) + (k - 1) * slot_us;
        end
        if k == 1
            flag = true;
        else
            scheduled_success(u) = true;
            next_tick(u) = t_now + (k - 1) * slot_us;
        end
    end
    ...
    function process_timeout(u, t_now)
        ...
        if strcmp(mode,'sb_cb')
            ...
            % t_now + 34 us if the channel stays idle, then the Bernoulli
            % draw is made immediately at that instant.
            node_state(u) = ST_SENSE;
            sense_start(u) = t_now;
            next_tick(u) = t_now;
        else
            % Unslotted: retry on the next 9 us tick, as in the plan.
            node_state(u) = ST_READY;
            next_tick(u) = idle_resume_tick(t_now);
            if next_tick(u) <= t_now
                next_tick(u) = t_now + slot_us;
            end
        end
            ...
            if u == winner_id
                ...
                if ~tx_in_sector(u) && cts_min_sinr(u) >= cts_sinr_th
                    ...
                    if node_state(u) == ST_WAIT || ...
                        ...
                        scheduled_success(u) = false;
                        sense_count(u) = 0;
                        next_tick(u) = idle_resume_tick(winner_data_end);
                        if next_tick(u) <= t_now
                            next_tick(u) = t_now + slot_us;
                        end
                    end
            ...
            case AP_DATA
                ...
                if winner_id > 0
                    ...
                    if queue_count(winner_id) > 0
                        ...
                        scheduled_success(winner_id) = false;
                        sense_count(winner_id) = 0;
                        next_tick(winner_id) = idle_resume_tick(t_now);
    ...
    function nt = idle_resume_tick(t_resume)
        nt = ceil(t_resume / slot_us) * slot_us;
    end
...
function k = geometric_q(q, stream)
    if q >= 1
        k = 1;
        return;
    end
