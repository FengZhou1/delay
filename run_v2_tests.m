...
function test_mmw_integer_slot_parameters()
    ...
    assert(cfg.data_sinr_th_db==20 && cfg.cts_sinr_th_db==6, ...
        'The DATA/CTS SINR thresholds are not 20 dB and 6 dB.');
...
function test_sb_cb_separate_cts_data_thresholds()
    ...
    data_fail.PHY.AP_Rx_Matrix(:) = noise_w*10^(19/10);
    ...
    assert(low_data.diagnostics.rts_success>=1 && ...
        ...
        'A 19-dB DATA frame passed the 20-dB DATA threshold.');

    success = scenario;
    success.PHY.AP_Sector_Tx_Matrix(:) = noise_w*10^(10/10);
    success.PHY.AP_Rx_Matrix(:) = noise_w*10^(25/10);
    ...
    assert(isfinite(passed.packet_log.completion_us) && ...
           ...
           passed.diagnostics.data_sinr_th_db==20, ...
        'Separate 6-dB CTS and 20-dB DATA thresholds were not applied.');
...
function test_analysis_protocol_q_grid_and_filter()
    ...
    assert(isequal([sf_cb.condition.tuning.coarse_grid.q], ...
        cfg.q_coarse), ...
        'SF-CB protocol did not use the unified coarse q grid.');
    assert(isequal([sb_cf.condition.tuning.coarse_grid.q], ...
        cfg.q_coarse), ...
        'SB-CF protocol did not use the unified coarse q grid.');
    assert(isempty(sf_cb.condition.tuning.refined_grid) && ...
           isempty(sb_cf.condition.tuning.refined_grid), ...
        'Single-stage analysis tuning unexpectedly created a fine grid.');
