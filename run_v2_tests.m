...
function test_sf_cf_single_and_boundary()
    ...
    trace = make_manual_arrival_trace([0;198],[1;1],cfg);
    result = simulate_aloha_v2('sf_cf',trace,scenario,cfg,1,1,11);
    assert_equal(result.packet_log.hol_us,[0;198],'SF-CF HOL timing');
    assert_equal(result.packet_log.first_attempt_us,[0;198], ...
                 'SF-CF boundary decision ordering');
    assert_equal(result.packet_log.completion_us,[198;396], ...
...
function test_theory_boundary_trace_is_explicit()
    ...
    trace=generate_boundary_arrival_trace(30,cfg,125,198);
    assert(trace.theory_boundary_only && all(mod(trace.times_us,198)==0), ...
        'Theory-only arrivals were not aligned to 198-us boundaries.');
    assert(trace.tick_us==198 && cfg.arrival_tick_us==9, ...
...
function test_single_packet_M_endpoints()
    ...
    for M=[1 6]
        ...
        assert_equal(sf_cb.packet_log.completion_us,198+tp, ...
            sprintf('SF-CB M=%d reservation plus payload',M));
        assert_equal(sb_cf.packet_log.completion_us,36+tp, ...
            sprintf('SB-CF M=%d DIFS plus payload',M));
        assert_equal(sb_cb.packet_log.completion_us,234+tp, ...
...
function test_sf_cb_exact_payload()
    ...
    assert_equal(result.packet_log.completion_us,594, ...
                 'SF-CB must use one 198-us reservation plus exact payload');
...
function test_sb_cf_fresh_difs()
    ...
    assert_equal(result.packet_log.hol_us,[0;234], ...
                 'SB-CF next-HOL timing');
    assert_equal(result.packet_log.first_attempt_us,[36;270], ...
                 'SB-CF fresh full-DIFS timing');
    assert_equal(result.packet_log.completion_us,[234;468], ...
...
function test_sb_cf_classic_collision()
    ...
    assert(result.diagnostics.simultaneous_start_events>=1 && ...
           result.diagnostics.classic_collision_events>=1, ...
           'SB-CF classic simultaneous collision was not recorded.');
    assert(all(~isfinite(result.packet_log.completion_us)), ...
           'SB-CF decoded a frame from two simultaneous transmitters.');
    assert(result.diagnostics.capture_opportunities==0 && ...
           result.diagnostics.capture_successes==0 && ...
           ~result.diagnostics.data_sinr_used, ...
    ...
    assert(all(~isfinite(late.packet_log.completion_us)) && ...
           late.diagnostics.partial_collisions>=1 && ...
           late.diagnostics.late_start_attempts>=1, ...
        'A one-slot-late SB-CF DATA overlap did not invalidate both frames.');
...
function test_sb_cb_timing_and_collision()
    ...
    % RTS18 + SIFS18 + 8-sector CTS scan144 + SIFS18 + payload198.
    assert_equal(result.packet_log.completion_us,432, ...
    ...
    assert(result2.diagnostics.rts_capture_success==0 && ...
           ~result2.diagnostics.rts_sinr_used, ...
...
function test_sb_cb_separate_cts_data_thresholds()
    ...
    assert(low_cts.diagnostics.rts_success>=1 && ...
           low_cts.diagnostics.icr_miss_low_sinr>=1 && ...
...
function test_s7_exact_timing()
    ...
    assert_equal(result.packet_log.completion_us,396, ...
...
function test_sb_cb_counterfactual_cca()
    ...
    assert(missed.diagnostics.cca_eligible_fn>0, ...
        'A hidden classic RTS collision was not counted as a CCA false negative.');
    assert(missed.diagnostics.cca_eligible_fp==0 && ...
           missed.diagnostics.cca_eligible_fp + ...
           missed.diagnostics.cca_eligible_tn == ...
           missed.diagnostics.cca_eligible_decodable_negative, ...
...
function test_sb_cb_cts_halfduplex_and_late_data()
    ...
    assert(result.diagnostics.data_failure_transaction_delay_us>=396, ...
    ...
    assert(any(node2_starts==72) && any(node2_starts==288), ...
...
function test_sb_cb_nav_exact_expiry()
    ...
    assert(~isempty(node3_starts) && node3_starts(1)==468, ...
        'The bystander transmitted before NAV expiry plus a fresh 36-us DIFS.');
    assert_delay_and_conservation(result,trace);
end
...
function test_sb_cb_response_timeout()
    ...
    assert_equal(result.packet_log.collision_delay_us,[180;180], ...
