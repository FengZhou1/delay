...
%VALIDATE_EXPERIMENT_CONFIG Validate and normalize a v2 experiment config.
    ...
    if cfg.arrival_tick_us ~= timing.SLOT_US
        error('validate_experiment_config:ArrivalTickMismatch', ...
            ['All v2 protocols must share the mmWave physical grid: ', ...
             'arrival_tick_us must equal mmw_slot_us (%g us).'], ...
            timing.SLOT_US);
    end
    % Derived fields are refreshed after any user override so the saved
    ...
    if ~isscalar(cfg.mmw_data_slot_us) || ~isfinite(cfg.mmw_data_slot_us) || ...
            cfg.mmw_data_slot_us <= 0
        error('validate_experiment_config:BadDataSlot', ...
            'cfg.mmw_data_slot_us must be a positive finite scalar in us.');
    end
    ...
    if cfg.q_coarse_tune_runs < 1 || ...
            cfg.q_coarse_tune_runs ~= round(cfg.q_coarse_tune_runs) || ...
            cfg.q_fine_tune_runs < 1 || ...
            cfg.q_fine_tune_runs ~= round(cfg.q_fine_tune_runs)
        error('validate_experiment_config:BadQTuneRunCount', ...
              'q coarse/fine tuning run counts must be positive integers.');
    end
    ...
    if cfg.q_fine_points < 3 || cfg.q_fine_points ~= round(cfg.q_fine_points)
        error('validate_experiment_config:BadFinePointCount', ...
              'q_fine_points must be an integer greater than or equal to three.');
    end
    if cfg.q_max_refinement_passes < 1 || ...
            cfg.q_max_refinement_passes ~= round(cfg.q_max_refinement_passes)
        error('validate_experiment_config:BadMaxRefinementPasses', ...
              'q_max_refinement_passes must be a positive integer.');
    end
    if cfg.q_refine_neighbor_span < 1 || ...
            cfg.q_refine_neighbor_span ~= round(cfg.q_refine_neighbor_span)
        error('validate_experiment_config:BadRefineNeighborSpan', ...
              'q_refine_neighbor_span must be a positive integer.');
    end
    if cfg.q_preferred_neighbor_radius < 1 || ...
            cfg.q_preferred_neighbor_radius ~= round(cfg.q_preferred_neighbor_radius)
        error('validate_experiment_config:BadPreferredNeighborRadius', ...
              'q_preferred_neighbor_radius must be a positive integer.');
    end
    if cfg.q_validation_runs < 0 || ...
            cfg.q_validation_runs ~= round(cfg.q_validation_runs) || ...
            cfg.q_validation_max_candidates < 1 || ...
            cfg.q_validation_max_candidates ~= ...
                round(cfg.q_validation_max_candidates)
        error('validate_experiment_config:BadQValidationCount', ...
            ['q_validation_runs must be a non-negative integer and ', ...
              'q_validation_max_candidates must be a positive integer.']);
    end
    if cfg.q_coarse_seed_count < 1 || ...
            cfg.q_coarse_seed_count ~= round(cfg.q_coarse_seed_count) || ...
            cfg.q_refine_windows < 1 || ...
            cfg.q_refine_windows ~= round(cfg.q_refine_windows) || ...
            cfg.q_refine_points_per_window < 3 || ...
            cfg.q_refine_points_per_window ~= round(cfg.q_refine_points_per_window) || ...
            cfg.q_candidate_seed_count < 1 || ...
            cfg.q_candidate_seed_count ~= round(cfg.q_candidate_seed_count)
        error('validate_experiment_config:BadMultiBasinQConfig', ...
              ['Multi-basin q scan counts must be positive integers; ', ...
               'refine points must be at least three.']);
    end
    cfg.q_multi_basin_tuning = logical(cfg.q_multi_basin_tuning);
    if 2*cfg.q_preferred_neighbor_radius+1 > cfg.q_fine_points
        error('validate_experiment_config:NeighborRadiusExceedsFineGrid', ...
            ['q_fine_points must be at least ', ...
             '2*q_preferred_neighbor_radius+1.']);
    end
    if ~isscalar(cfg.q_refine_floor) || ~isfinite(cfg.q_refine_floor) || ...
            cfg.q_refine_floor <= 0 || cfg.q_refine_floor >= 1
        error('validate_experiment_config:BadRefineFloor', ...
              'q_refine_floor must be a finite scalar strictly between zero and one.');
    end
    ...
    cfg.cts_mode = lower(char(cfg.cts_mode));
    if ~ismember(cfg.cts_mode,{'sector_sweep','quasi_omni_ideal', ...
            'quasi_omni_physical','quasi_omni_isotropic', ...
            'directional_winner'})
        error('validate_experiment_config:BadCtsMode', ...
              'Unsupported cts_mode: %s.',cfg.cts_mode);
    end
    if ~isscalar(cfg.qo_peak_gain_db) || ~isfinite(cfg.qo_peak_gain_db)
        error('validate_experiment_config:BadQoPeakGain', ...
              'qo_peak_gain_db must be a finite scalar.');
    end
    if numel(cfg.qo_amplitude_taper) ~= cfg.mmw_real_n_sectors && ...
            numel(cfg.qo_amplitude_taper) ~= 4
        error('validate_experiment_config:BadQoTaper', ...
              'qo_amplitude_taper must contain one weight per ULA element.');
    end
    cfg.qo_amplitude_taper = double(cfg.qo_amplitude_taper(:).');
    if any(~isfinite(cfg.qo_amplitude_taper) | cfg.qo_amplitude_taper <= 0)
        error('validate_experiment_config:BadQoTaperValues', ...
              'qo_amplitude_taper weights must be finite and positive.');
    end
    cfg.data_failure_mode = lower(char(cfg.data_failure_mode));
    if ~ismember(cfg.data_failure_mode,{'txop','packet'})
        error('validate_experiment_config:BadDataFailureMode', ...
    ...
    logical_fields = {'parallel','resume','collect_packet_log','collect_diagnostics', ...
        ...
        'q_two_stage_tuning','q_require_stable_neighbors', ...
        'q_fallback_self_stable','common_arrivals_by_effective_rate'};
    ...
    cfg.txop_mode = lower(char(cfg.txop_mode));
    if strcmp(cfg.txop_mode, 'batch_m')
        cfg.txop_mode = 'batch_M';
    end
    if ~ismember(cfg.txop_mode, {'ready_queue', 'batch_M'})
        error('validate_experiment_config:BadTxopMode', ...
    ...
    cfg.q_refine_scale = lower(char(cfg.q_refine_scale));
    if ~ismember(cfg.q_refine_scale,{'auto','linear','log'})
        error('validate_experiment_config:BadRefineScale', ...
