    ...
    for ci = 1:numel(conditions)
        ...
        if strcmp(info.protocol, 'sb_cf')
            row.cca_truth_definition = ...
                "AP single-user lock or a start that is physically undecodable/harmful";
        else
            row.cca_truth_definition = ...
                "counterfactual start corrupts an existing decodable RTS/CTS/data reception";
        end
...
function table_out = empty_csma_table()
    names = {'protocol','load_mode','lambda_base','lambda_effective','M','Tp_us', ...
        ...
        'capture_successes','partial_collision_events','full_collision_events', ...
...
function write_chinese_report(path, output_dir, cfg, summary, theory, capacity, csma, ...
    ...
    line('- `csma_diagnostics.csv`：从评估检查点按条件汇总的 CCA、晚启动、捕获、ICR、NAV 和碰撞诊断；`collision_channel_time_us` 是碰撞区间并集的墙钟时间，`collision_tx_airtime_us` 是失败发送者空口时间之和，后者在多发送者碰撞时可以更大。\n');
